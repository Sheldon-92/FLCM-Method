...
class RICEFramework {
    ...
    calculate(content, metadata) {
        ...
        const total = ((relevance * weights.relevance) *
     ...
     */
    calculateRelevance(content, metadata) {
        let score = 50; // Base score
        // Check for keywords
        if (this.config.keywords && this.config.keywords.length > 0) {
            const contentLower = content.toLowerCase();
            const keywordMatches = this.config.keywords.filter(keyword => contentLower.includes(keyword.toLowerCase()));
        ...
        const techMatches = technicalTerms.filter(term => content.toLowerCase().includes(term));
        score += Math.min(15, techMatches.length * 3);
        // Check metadata signals
        if (metadata?.source === 'academic')
            score += 10;
        if (metadata?.source === 'industry')
            score += 8;
     ...
     */
    calculateImpact(content, metadata) {
    ...
    calculateConfidence(content, metadata) {
        ...
        if (metadata?.verified)
            score += 10;
        if (metadata?.authorCredentials)
            score += 10;
        if (metadata?.peerReviewed)
            score += 15;
        return Math.min(100, Math.max(0, score));
    }
    /**
     ...
     */
    calculateEffort(content, metadata) {
        ...
        if (content.match(complexityIndicators.technicalJargon)?.length > 10)
            score += 15;
        if (content.match(complexityIndicators.nestedStructure)?.length > 5)
            score += 10;
        if (content.match(complexityIndicators.specialCharacters)?.length > 20)
            score += 5;
        if (content.match(complexityIndicators.codeBlocks)?.length > 0)
            score += 10;
        ...
        if (metadata?.hasImages)
            score += 10;
        if (metadata?.hasVideos)
            score += 15;
        if (metadata?.requiresTranslation)
            score += 20;
        return Math.min(100, Math.max(0, score));
    }
    /**
