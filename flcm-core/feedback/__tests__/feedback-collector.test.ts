...
import { FeedbackCollector, FeedbackEntry, FeedbackCategory, FeedbackPriority } from '../feedback-collector';
...
describe('FeedbackCollector', () => {
  ...
  describe('Constructor', () => {
    test('should initialize properly', () => {
      ...
      expect(feedbackCollector).toBeInstanceOf(FeedbackCollector);
    });
    ...
    test('should accept configuration options', () => {
      ...
      const customCollector = new FeedbackCollector(config);
      expect(customCollector).toBeInstanceOf(FeedbackCollector);
    });
  });
  ...
  describe('collectFeedback', () => {
    test('should collect basic user feedback', async () => {
      const feedback = await feedbackCollector.collectFeedback(
        ...
        'The generated content was excellent!',
        5
      );
      ...
      expect(feedback.category).toBe(FeedbackCategory.CONTENT_QUALITY);
    ...
    test('should handle feedback with context', async () => {
      ...
      const feedback = await feedbackCollector.collectFeedback(
        ...
        'Agent performed well but could be faster',
        4,
        context
      );
    ...
    test('should auto-categorize feedback based on content', async () => {
      const performanceFeedback = await feedbackCollector.collectFeedback(
        'user123',
        FeedbackCategory.GENERAL,
        'The system is running very slowly today',
        2
      );

      const contentFeedback = await feedbackCollector.collectFeedback(
        'user123',
        FeedbackCategory.GENERAL,
        'The generated article was well-written and engaging',
        5
      );

      // Should auto-categorize based on keywords
      expect(performanceFeedback.suggestedCategory).toBe(FeedbackCategory.PERFORMANCE);
      expect(contentFeedback.suggestedCategory).toBe(FeedbackCategory.CONTENT_QUALITY);
    });
  });
  ...
  describe('getFeedback', () => {
    beforeEach(async () => {
      ...
      await feedbackCollector.collectFeedback('user1', FeedbackCategory.CONTENT_QUALITY, 'Great content!', 5);
      await feedbackCollector.collectFeedback('user2', FeedbackCategory.PERFORMANCE, 'Too slow', 2);
      await feedbackCollector.collectFeedback('user1', FeedbackCategory.USABILITY, 'Easy to use', 4);
    });
    ...
    test('should filter feedback by category', () => {
      ...
      expect(contentFeedback[0].category).toBe(FeedbackCategory.CONTENT_QUALITY);
    });
  ...
  describe('analyzeFeedback', () => {
    beforeEach(async () => {
      ...
      await feedbackCollector.collectFeedback('user1', FeedbackCategory.CONTENT_QUALITY, 'Amazing results!', 5);
      await feedbackCollector.collectFeedback('user2', FeedbackCategory.CONTENT_QUALITY, 'Very good quality', 4);
      await feedbackCollector.collectFeedback('user3', FeedbackCategory.CONTENT_QUALITY, 'Content is okay', 3);
      await feedbackCollector.collectFeedback('user4', FeedbackCategory.PERFORMANCE, 'Fast execution', 5);
      await feedbackCollector.collectFeedback('user5', FeedbackCategory.PERFORMANCE, 'Too slow!', 2);
      await feedbackCollector.collectFeedback('user6', FeedbackCategory.USABILITY, 'Hard to understand', 2);
    });
    ...
    beforeEach(async () => {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        await feedbackCollector.collectFeedback(
          ...
          FeedbackCategory.CONTENT_QUALITY,
          'Test feedback',
          Math.round(Math.max(1, Math.min(5, rating))),
          {},
          timestamp
        );
      }
  ...
  describe('exportFeedback', () => {
    beforeEach(async () => {
      await feedbackCollector.collectFeedback('user1', FeedbackCategory.CONTENT_QUALITY, 'Test 1', 5);
      await feedbackCollector.collectFeedback('user2', FeedbackCategory.PERFORMANCE, 'Test 2', 3);
    });
    ...
    test('should export with filters applied', () => {
      ...
      expect(parsed.feedback[0].category).toBe(FeedbackCategory.CONTENT_QUALITY);
    });
  });
  ...
  describe('Sentiment Analysis', () => {
    test('should analyze sentiment in feedback text', async () => {
      const positiveFeedback = await feedbackCollector.collectFeedback(
        'user1',
        FeedbackCategory.CONTENT_QUALITY,
        'I absolutely love this amazing feature! It works perfectly and exceeds expectations.',
        5
      );

      const negativeFeedback = await feedbackCollector.collectFeedback(
        'user2',
        FeedbackCategory.USABILITY,
    ...
    test('should identify emotional keywords', async () => {
      const feedback = await feedbackCollector.collectFeedback(
        'user1',
        FeedbackCategory.GENERAL,
        'I am extremely disappointed with the slow performance and buggy interface',
        2
      );
  ...
  describe('User Satisfaction Tracking', () => {
    ...
    test('should calculate Net Promoter Score', () => {
      ...
      const feedbackData = [
        { rating: 5, category: FeedbackCategory.GENERAL },
        { rating: 5, category: FeedbackCategory.GENERAL },
        { rating: 4, category: FeedbackCategory.GENERAL },
        { rating: 3, category: FeedbackCategory.GENERAL },
        { rating: 2, category: FeedbackCategory.GENERAL },
        { rating: 1, category: FeedbackCategory.GENERAL }
      ];
  ...
  describe('Data Persistence', () => {
    ...
    test('should handle storage errors gracefully', async () => {
      ...
      jest.spyOn(feedbackCollector as any, 'saveFeedback').mockRejectedValueOnce(
