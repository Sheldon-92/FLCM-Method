...
const logger = (0, logger_1.createLogger)('MediaProcessor');
class MediaProcessor {
    ...
    detectMediaType(source) {
        if (typeof source === 'string') {
            if (source.match(/\.(mp4|avi|mov|mkv)$/i))
                return 'video';
            if (source.match(/\.(mp3|wav|ogg|m4a)$/i))
                return 'audio';
            if (source.match(/\.(jpg|jpeg|png|gif|bmp)$/i))
                return 'image';
        }
