...
describe('Content Generation Modes', () => {
    ...
    let mockVoiceProfile;
    let mockContentGenerator;
    let mockDialogueManager;
    beforeEach(() => {
        agent = new index_1.CreatorAgent();
        // Create mock insights document
        mockInsightsDocument = (0, document_schema_1.createInsightsDocument)({ type: 'text', path: 'test-source', hash: 'test-hash' }, ['SWOT-USED', 'Socratic'], 'Comprehensive analysis of AI implementation in business operations.', 'Scholar Agent');
        ...
        // Replace instances
        agent.contentGenerator = mockContentGenerator;
        agent.dialogueManager = mockDialogueManager;
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('Quick Mode (3-minute target)', () => {
        ...
        it('should handle minimal insights gracefully', async () => {
            const minimalInsights = (0, document_schema_1.createInsightsDocument)({ type: 'text', path: 'minimal', hash: 'minimal' }, ['SWOT-USED'], 'Brief analysis', 'Scholar Agent');
      ...
      `);
        ...
        it('should respect framework selection', async () => {
            ...
            for (const framework of frameworks) {
                ...
                expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ framework }));
            }
        });
        it('should handle different tone settings', async () => {
            ...
            for (const tone of tones) {
                ...
                await agent.create(mockInsightsDocument, options);
                expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ tone }));
            }
        ...
        it('should approximate target word count', async () => {
            ...
            for (const targetWordCount of wordCounts) {
                ...
                await agent.create(mockInsightsDocument, options);
                expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ targetWords: targetWordCount }));
            }
        });
        it('should use default framework when not specified', async () => {
            ...
            await agent.create(mockInsightsDocument, options);
            expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ framework: 'analytical' }));
        });
        it('should use default word count when not specified', async () => {
            ...
            await agent.create(mockInsightsDocument, options);
            expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ targetWords: 1500 }));
        });
        it('should use default tone when not specified', async () => {
            ...
            await agent.create(mockInsightsDocument, options);
            expect(mockContentGenerator.generate).toHaveBeenCalledWith(expect.objectContaining({ tone: 'professional' }));
        });
    });
    describe('Custom Mode (Interactive)', () => {
      ...
      `);
        ...
        it('should incorporate feedback effectively', async () => {
            ...
            await agent.create(mockInsightsDocument, options);
            expect(mockContentGenerator.refine).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                satisfied: false,
                suggestions: [
                    'Add implementation timeline',
                    'Include cost analysis',
    ...
    describe('Mode Comparison and Performance', () => {
        it('should show performance differences between modes', async () => {
            const modes = ['quick', 'standard', 'custom'];
            const durations = {};
        ...
        it('should produce appropriate content length by mode', async () => {
            const results = {};
            for (const mode of ['quick', 'standard', 'custom']) {
                const options = {
                    mode,
                    voiceProfile: mockVoiceProfile,
                };
        ...
        it('should handle mode switching gracefully', async () => {
            const modeSequence = ['quick', 'standard', 'custom', 'quick'];
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle invalid mode gracefully', async () => {
            ...
            await expect(agent.create(mockInsightsDocument, options)).rejects.toThrow('Unknown creation mode: invalid-mode');
        });
...
//# sourceMappingURL=content-generation-modes.test.js.map