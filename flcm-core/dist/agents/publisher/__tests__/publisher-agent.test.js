...
const createMockAdapter = (platform, optimization = 90) => ({
    platform,
...
describe('PublisherAgent', () => {
    ...
    beforeEach(() => {
        agent = new index_1.PublisherAgent();
        // Create mock content document
        mockContentDocument = (0, document_schema_1.createContentDocument)('insights-123', 'voice-profile-456', 'standard', `# AI Implementation Strategy
      ...
      ## Conclusion
      Strategic AI implementation transforms business operations when executed with comprehensive planning and stakeholder engagement.`, 'Creator Agent');
    ...
    describe('Platform Adaptation', () => {
        ...
        it('should throw error for unsupported platform', async () => {
            await expect(agent.adaptForPlatform(mockContentDocument, 'unsupported')).rejects.toThrow('No adapter available for platform: unsupported');
        });
        it('should handle adapter failures gracefully', async () => {
            const mockAdapter = agent.adapters.get('xiaohongshu');
            mockAdapter.adapt.mockRejectedValue(new Error('Adaptation failed'));
            await expect(agent.adaptForPlatform(mockContentDocument, 'xiaohongshu')).rejects.toThrow('Adaptation failed');
        });
    });
    describe('Content Optimization', () => {
        it('should optimize hashtags for different platforms', () => {
            ...
            const platforms = ['xiaohongshu', 'zhihu', 'wechat', 'linkedin'];
        ...
        it('should generate default hashtags for unsupported platforms', () => {
            ...
            const hashtags = agent.optimizeHashtags(content, 'unsupported');
            expect(hashtags).toBeDefined();
            expect(hashtags.length).toBeGreaterThan(0);
    ...
    describe('Multi-Platform Publishing', () => {
        ...
        it('should handle partial publishing failures', async () => {
            ...
            const mockAdapter = agent.adapters.get('zhihu');
        ...
        it('should apply optimization when requested', async () => {
            ...
            const adapter = agent.adapters.get('xiaohongshu');
    ...
    describe('Scheduling Management', () => {
        ...
        it('should calculate optimal publishing times', async () => {
            await agent.schedule(mockContentDocument, { optimalTime: true }, ['xiaohongshu', 'zhihu']);
            const queue = agent.getQueue();
        ...
        it('should schedule for next day if optimal time has passed', async () => {
            ...
            await agent.schedule(mockContentDocument, { optimalTime: true }, ['xiaohongshu']);
            const queue = agent.getQueue();
        ...
        it('should clean up old published posts', async () => {
            ...
            agent.publishQueue.push(oldPost);
            await agent.processQueue();
            const queue = agent.getQueue();
    ...
    describe('Error Handling and Edge Cases', () => {
        ...
        it('should validate platform availability', async () => {
            const invalidPlatforms = ['facebook', 'twitter', 'instagram'];
            const options = {
        ...
        it('should handle very long content appropriately', async () => {
            const longContent = (0, document_schema_1.createContentDocument)('long-insights', 'voice-profile', 'standard', 'Very long content. '.repeat(1000), 'Creator Agent');
            const options = {
                platforms: ['xiaohongshu'],
                optimize: true,
            };
        ...
        it('should handle special characters in content', async () => {
            const specialContent = (0, document_schema_1.createContentDocument)('special-insights', 'voice-profile', 'standard', 'Content with émojis 🚀 and spëcial châractérs!', 'Creator Agent');
            const options = {
                platforms: ['xiaohongshu'],
            };
    ...
    describe('Performance and Statistics', () => {
        it('should track publishing statistics correctly', async () => {
            ...
            const stats = agent.publishingStats;
        ...
        it('should handle concurrent publishing requests', async () => {
            const options1 = { platforms: ['xiaohongshu'] };
            const options2 = { platforms: ['zhihu'] };
            const promises = [
    ...
    describe('Agent Lifecycle', () => {
        ...
        it('should shutdown correctly', async () => {
            ...
            const adapters = agent.adapters;
            const queue = agent.publishQueue;
            expect(adapters.size).toBe(0);
            expect(queue).toHaveLength(0);
        });
    });
});
//# sourceMappingURL=publisher-agent.test.js.map