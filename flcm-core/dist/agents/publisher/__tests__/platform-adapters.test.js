...
describe('Platform Adapters', () => {
    let mockContentDocument;
    beforeEach(() => {
        mockContentDocument = (0, document_schema_1.createContentDocument)('insights-123', 'voice-profile-456', 'standard', `# Comprehensive Guide to AI in Business
...
## Conclusion
Strategic AI implementation transforms business operations when executed with comprehensive planning, adequate investment, and proper change management. Organizations that approach AI adoption systematically achieve better outcomes and sustainable competitive advantages.`, 'Creator Agent');
    ...
    describe('XiaoHongShu Adapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new xiaohongshu_adapter_1.XiaohongshuAdapter();
        });
        ...
        it('should optimize content for XiaoHongShu characteristics', async () => {
            ...
            expect(platformContent.hashtags.some(tag => tag.includes('生活') || tag.includes('分享') || tag.includes('AI') || tag.includes('科技'))).toBe(true);
        });
        ...
        it('should handle Chinese content appropriately', async () => {
            const chineseContent = (0, document_schema_1.createContentDocument)('insights-cn', 'voice-profile', 'standard', '人工智能在商业中的应用正在改变我们的工作方式。企业通过AI技术提高效率，降低成本，创造更多价值。', 'Creator Agent');
    ...
    describe('Zhihu Adapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new zhihu_adapter_1.ZhihuAdapter();
        });
        it('should have correct platform identifier', () => {
        ...
        it('should optimize content for Zhihu characteristics', async () => {
            ...
            expect(platformContent.hashtags.some(tag => tag.includes('人工智能') || tag.includes('商业') || tag.includes('技术'))).toBe(true);
        });
        ...
        it('should maintain technical depth for Zhihu audience', async () => {
            const technicalContent = (0, document_schema_1.createContentDocument)('technical-insights', 'voice-profile', 'standard', 'Neural networks utilize backpropagation algorithms for gradient descent optimization.', 'Creator Agent');
    ...
    describe('WeChat Adapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new wechat_adapter_1.WechatAdapter();
        });
        it('should have correct platform identifier', () => {
        ...
        it('should format content for mobile reading', async () => {
            const longContent = (0, document_schema_1.createContentDocument)('long-content', 'voice-profile', 'standard', 'Very long paragraph without breaks. '.repeat(50), 'Creator Agent');
    ...
    describe('LinkedIn Adapter', () => {
        let adapter;
        beforeEach(() => {
            adapter = new linkedin_adapter_1.LinkedInAdapter();
        });
        it('should have correct platform identifier', () => {
        ...
        it('should optimize content for LinkedIn characteristics', async () => {
            ...
            // Should include professional hashtags
            expect(platformContent.hashtags.some(tag => tag.toLowerCase().includes('business') ||
                tag.toLowerCase().includes('ai') ||
                tag.toLowerCase().includes('technology'))).toBe(true);
        ...
        it('should suggest professional visuals for LinkedIn', () => {
            ...
            const professionalVisual = visuals.find(v => v.style.toLowerCase().includes('professional'));
            expect(professionalVisual).toBeDefined();
            const infographicVisual = visuals.find(v => v.type === 'infographic');
            expect(infographicVisual).toBeDefined();
            // Should include business elements
            visuals.forEach(visual => {
                expect(visual.elements).toContain('title');
                expect(visual.elements.some(e => e.includes('logo') || e.includes('brand') || e.includes('data'))).toBe(true);
            });
        });
        ...
        it('should maintain professional tone', async () => {
            const casualContent = (0, document_schema_1.createContentDocument)('casual-content', 'voice-profile', 'standard', 'Hey everyone! This AI stuff is really cool and awesome!', 'Creator Agent');
        ...
        it('should format content for business context', async () => {
            const technicalContent = (0, document_schema_1.createContentDocument)('technical-content', 'voice-profile', 'standard', 'Neural networks use backpropagation for training with gradient descent optimization.', 'Creator Agent');
            const platformContent = await adapter.adapt(technicalContent);
    ...
    describe('Cross-Platform Compatibility', () => {
        ...
        beforeEach(() => {
            adapters = [
                new xiaohongshu_adapter_1.XiaohongshuAdapter(),
                new zhihu_adapter_1.ZhihuAdapter(),
                new wechat_adapter_1.WechatAdapter(),
                new linkedin_adapter_1.LinkedInAdapter(),
            ];
        });
        ...
        it('should handle optimization consistently', async () => {
            for (const adapter of adapters) {
                const originalContent = await adapter.adapt(mockContentDocument);
                const optimizedContent = await adapter.optimize(originalContent);
                expect(optimizedContent.metadata.optimizationScore).toBeGreaterThanOrEqual(originalContent.metadata.optimizationScore);
    ...
    describe('Performance and Error Handling', () => {
        ...
        it('should handle empty content gracefully', async () => {
            const emptyContent = (0, document_schema_1.createContentDocument)('empty-insights', 'voice-profile', 'standard', '', 'Creator Agent');
            emptyContent.title = '';
            for (const adapter of adapters) {
        ...
        it('should handle very long content appropriately', async () => {
            const longContent = (0, document_schema_1.createContentDocument)('long-insights', 'voice-profile', 'standard', 'Very long content section. '.repeat(200), 'Creator Agent');
            for (const adapter of adapters) {
                const platformContent = await adapter.adapt(longContent);
                expect(platformContent).toBeDefined();
        ...
        it('should handle special characters and unicode', async () => {
            const specialContent = (0, document_schema_1.createContentDocument)('special-insights', 'voice-profile', 'standard', 'Content with émojis 🚀 and special châractérs! 中文内容测试 #hashtag', 'Creator Agent');
            for (const adapter of adapters) {
...
//# sourceMappingURL=platform-adapters.test.js.map