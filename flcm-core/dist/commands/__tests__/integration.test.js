...
accurate, and accessible medical care for patients worldwide.
    ...
    describe('Help and Status Commands', () => {
        test('should execute help command successfully', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:help');
        ...
        test('should execute status command successfully', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:status');
            expect(result.success).toBe(true);
        ...
        test('should show command history', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:history');
            expect(result.success).toBe(true);
    ...
    describe('Scholar Command Integration', () => {
        test('should execute scholar command with text input', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:scholar', [], {
                text: testContent,
        ...
        test('should show scholar help', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:scholar', [], { help: true });
            expect(result.success).toBe(true);
    ...
    describe('Creator Command Integration', () => {
        test('should execute creator command with topic', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:create', [], {
        ...
        test('should show creator help', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:create', [], { help: true });
            expect(result.success).toBe(true);
...
Artificial Intelligence is revolutionizing healthcare by improving diagnostic accuracy and personalizing treatment plans.
      `.trim();
        ...
        test('should execute publisher command with single platform', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:publish', [], {
                content: draftFile,
                platform: 'linkedin',
                output: testOutputDir
            });
        ...
        test('should show publisher help', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:publish', [], { help: true });
            expect(result.success).toBe(true);
    ...
    describe('Workflow Command Integration', () => {
        test('should execute quick workflow with topic', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:quick', [], {
        ...
        test('should execute standard workflow with file input', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:standard', [], {
                input: testFilePath,
                platform: 'linkedin',
                output: testOutputDir
            });
        ...
        test('should execute full flow workflow', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:flow', [], {
        ...
        test('should show workflow help', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:flow', [], { help: true });
            expect(result.success).toBe(true);
    ...
    describe('Error Handling', () => {
        test('should handle unknown command', async () => {
            const result = await (0, index_1.executeFLCMCommand)('flcm:unknown');
            expect(result.success).toBe(false);
    ...
    describe('End-to-End Workflow Test', () => {
        test('should complete full content creation pipeline', async () => {
            ...
            const creatorResult = await (0, index_1.executeFLCMCommand)('flcm:create', [], {
                input: analysisPath.replace('.md', '.json'),
                mode: 'quick',
                output: workflowDir
            });
            ...
            publishResult.data.outputPaths.forEach((filePath) => {
                expect(fs.existsSync(filePath)).toBe(true);
            });
            // Compare with integrated workflow
            const integratedResult = await (0, index_1.executeFLCMCommand)('flcm:flow', [], {
                text: testContent,
                platform: 'linkedin,wechat',
 ...
 */
function createTestContent(topic, length = 'medium') {
    const baseContent = `
# ${topic}
