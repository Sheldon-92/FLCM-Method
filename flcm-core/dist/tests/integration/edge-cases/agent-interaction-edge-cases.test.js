...
jest.mock('os', () => ({
    freemem: jest.fn().mockReturnValue(1024 * 1024 * 1024),
...
describe('Agent Interaction Edge Cases', () => {
    ...
    beforeAll(() => {
        ...
        recoveryManager = new recovery_manager_1.RecoveryManager();
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('数据传递异常测试', () => {
        it('should handle Scholar output format corruption', async () => {
            ...
            try {
                ...
                const result = await creatorAgent.create(mockCorruptedInsights, {
                    mode: 'quick',
                });
        ...
        it('should handle Creator output incompleteness', async () => {
            ...
            try {
                ...
                const results = await publisherAgent.publish(mockIncompleteContent, {
    ...
    describe('性能极限测试', () => {
        ...
        it('should handle concurrent multi-user scenarios', async () => {
            ...
            try {
                ...
                const results = await Promise.all(userRequests.map(async (request) => {
        ...
        it('should handle network delay and timeout scenarios', async () => {
            console.log('\n=== 测试网络延迟和超时场景 ===');
            // Mock network delays
            const mockNetworkDelay = (ms) => new Promise(resolve => setTimeout(resolve, ms));
    ...
    describe('错误恢复测试', () => {
        ...
        it('should handle resource exhaustion gracefully', async () => {
            ...
            const resourceScenarios = [
                { type: 'memory', available: 100 * 1024 * 1024 },
                { type: 'cpu', available: 0.1 },
            ...
            for (const scenario of resourceScenarios) {
                ...
                const degradedResult = await recoveryManager.handleResourceExhaustion({
                    ...
                    requestedAmount: scenario.available * 2,
    ...
    describe('跨平台兼容性测试', () => {
        ...
        it('should manage memory efficiently across environments', async () => {
            console.log('\n=== 测试跨环境内存管理 ===');
            const memoryTests = [
                { environment: 'low-memory', limit: 512 * 1024 * 1024 },
                { environment: 'high-memory', limit: 8 * 1024 * 1024 * 1024 },
...
//# sourceMappingURL=agent-interaction-edge-cases.test.js.map