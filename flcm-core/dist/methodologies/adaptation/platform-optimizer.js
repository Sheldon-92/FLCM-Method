...
exports.PlatformOptimizer = void 0;
class PlatformOptimizer {
    ...
    optimize(content, platform, keyMessage) {
        ...
        const engagementScore = this.calculateEngagementScore(optimizedContent, config, formattingApplied);
        return {
    ...
    trimContent(content, maxChars, keyMessage) {
    ...
    formatForTwitter(content, keyMessage) {
    ...
    formatForXiaohongshu(content) {
        ...
        formatted = sections.map((section, i) => {
            if (i === 0)
                return section; // Keep title as is
    ...
    addEngagementElements(content, config) {
        let modified = false;
        let result = content;
        const elements = [];
    ...
    generateSuggestions(platform, content) {
        const suggestions = [];
        if (platform === 'linkedin') {
    ...
    calculateEngagementScore(content, config, formattingApplied) {
        ...
        if (config.name === 'LinkedIn' && content.includes('#'))
            score += 5;
...
exports.default = PlatformOptimizer;
//# sourceMappingURL=platform-optimizer.js.map