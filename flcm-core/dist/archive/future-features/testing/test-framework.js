...
exports.FLCMTestFramework = void 0;
const logger_1 = require("../shared/utils/logger");
const events_1 = require("events");
class FLCMTestFramework extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.logger = new logger_1.Logger('FLCMTestFramework');
        this.config = config;
        this.id = config.id;
    ...
    async updateSuite(suiteId, suite) {
        try {
    ...
    async runTests(testIds, options = {}) {
        try {
    ...
    async removeReporter(reporterId) {
        try {
    ...
    async generateReport(runId, reporterId) {
        try {
    ...
    async createTestReporter(config) {
        return {
...
exports.FLCMTestFramework = FLCMTestFramework;
...
class DefaultTestSuiteManager {
    ...
    async processDependencies(suite) {
...
class DefaultTestRunManager {
    ...
    async executeRun(run, suites, runners) {
        this.logger.debug(`Executing test run: ${run.id}`);
...
class DefaultTestMockManager {
    ...
    async setupSuiteMocks(suite) {
        this.logger.debug(`Setting up mocks for suite: ${suite.id}`);
    }
    async cleanupSuiteMocks(suite) {
