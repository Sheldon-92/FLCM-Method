...
exports.VoiceDNAFramework = void 0;
const base_1 = require("../base");
class VoiceDNAFramework extends base_1.BaseFramework {
    constructor() {
        super();
...
- [ ] Avoids unwanted characteristics`;
    ...
    extractTone(selection) {
        const match = selection.match(/^([^-]+)/);
        return match ? match[1].trim().toLowerCase() : 'professional';
    }
    extractFormality(selection) {
        if (selection.includes('Very Formal'))
            return 'very formal';
        if (selection.includes('Professional'))
            return 'professional';
        if (selection.includes('Conversational'))
            return 'conversational';
        if (selection.includes('Very Casual'))
            return 'very casual';
        if (selection.includes('Casual'))
            return 'casual';
        return 'professional';
    }
    extractHumor(selection) {
        if (selection.includes('None'))
            return 'none';
        if (selection.includes('Occasional'))
            return 'occasional';
        if (selection.includes('Regular'))
            return 'regular';
        if (selection.includes('Frequent'))
            return 'frequent';
        return 'occasional';
    }
    parseTraits(input) {
    ...
    parseAvoidList(input) {
        return input.split(/[,;]/)
            .map(a => a.trim().toLowerCase())
            .filter(a => a.length > 0);
    }
    generateVoiceGuidelines(voiceDNA, answers) {
        const guidelines = [];
        // Tone guidelines
    ...
    calculateVoiceClarity(answers) {
        let clarity = 0;
        const totalFactors = 7;
        if (answers.natural_voice?.length > 50)
            clarity += 1 / totalFactors;
        if (answers.tone_preference)
            clarity += 1 / totalFactors;
        if (answers.personality_traits?.length > 20)
            clarity += 1 / totalFactors;
        if (answers.core_values?.length > 20)
            clarity += 1 / totalFactors;
        if (answers.audience_relationship?.length > 20)
            clarity += 1 / totalFactors;
        if (answers.avoid_characteristics?.length > 10)
            clarity += 1 / totalFactors;
        if (answers.formality_level)
            clarity += 1 / totalFactors;
...
exports.VoiceDNAFramework = VoiceDNAFramework;
//# sourceMappingURL=voice-dna-framework.js.map