...
exports.PyramidPrincipleFramework = void 0;
const base_1 = require("../base");
class PyramidPrincipleFramework extends base_1.BaseFramework {
    constructor() {
        super();
...
Let's build your pyramid structure.`;
    ...
    getQuestions(context) {
        ...
        return [
            ...
            {
                id: 'complication',
                question: 'What', s, the, COMPLICATION, What, has, changed, gone, wrong, or, needs, attention, ',: type, 'open': ,
                required: true,
            ...
            {
                id: 'main_point',
                question: 'What', s, your, MAIN, POINT, or, key, ',: type, 'open': ,
                required: true,
    ...
    organizeSupportingData(answers) {
        const data = {};
        for (let i = 1; i <= 5; i++) {
    ...
    checkMECE(arguments) {
        ...
        for (let i = 0; i < concepts.length - 1; i++) {
            for (let j = i + 1; j < concepts.length; j++) {
                const common = concepts[i].filter(word => concepts[j].includes(word) && word.length > 4);
                if (common.length > 2)
                    overlaps++;
            }
        }
    ...
    checkLogicalConsistency(pyramid, answers) {
        ...
        if (pyramid.logicalFlow === 'deductive') {
            ...
            return pyramid.keyArguments.some(arg => arg.toLowerCase().includes('therefore') ||
    ...
    checkAudienceAlignment(pyramid, answers) {
        if (!answers.audience)
            return false;
        ...
        return keywords.some(keyword => audienceConcerns.includes(keyword) && mainPoint.includes(keyword));
    }
    generateInsights(pyramid, analysis, answers) {
        const insights = [];
        // SCQ insights
    ...
    generateRecommendations(pyramid, analysis, answers) {
        const recommendations = [];
        // Structure recommendations
        if (!analysis.scqComplete) {
            if (!pyramid.situation)
                recommendations.push('Add clear situation context');
            if (!pyramid.complication)
                recommendations.push('Identify the complication driving action');
            if (!pyramid.question)
                recommendations.push('Clarify the implied question');
        }
    ...
    generateNextSteps(pyramid, answers) {
        const steps = [];
        // Document creation
    ...
    createCommunicationTemplates(pyramid, answers) {
        return {
    ...
    createExecutiveSummary(pyramid, answers) {
...
Next Steps: ${answers.next_steps || 'To be determined'}`;
    ...
    createEmailTemplate(pyramid, answers) {
...
Please let me know if you need additional information.`;
    ...
    createPresentationOutline(pyramid) {
...
${pyramid.keyArguments.map((arg, i) => `Slide ${i + 4}: Key Point ${i + 1}\n- ${arg}\n- Supporting evidence`).join('\n\n')}

Final Slide: Next Steps & Questions`;
    }
    createElevatorPitch(pyramid, answers) {
...
(Delivered in 30 seconds)`;
    ...
    calculateClarityScore(pyramid, analysis) {
        let score = 0;
        // SCQ completeness (0.3)
        if (analysis.scqComplete)
            score += 0.3;
        else
            score += 0.3 * (Object.values({
                s: pyramid.situation,
        ...
        if (analysis.logicalConsistency)
            score += 0.1;
...
exports.PyramidPrincipleFramework = PyramidPrincipleFramework;
//# sourceMappingURL=pyramid-principle-framework.js.map