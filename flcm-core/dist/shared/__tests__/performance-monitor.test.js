...
describe('PerformanceMonitor', () => {
    ...
    describe('Constructor', () => {
        test('should initialize with default thresholds', () => {
            ...
            expect(performanceMonitor).toBeInstanceOf(performance_monitor_1.PerformanceMonitor);
        });
        test('should accept custom thresholds', () => {
            ...
            const customMonitor = new performance_monitor_1.PerformanceMonitor(customThresholds);
            expect(customMonitor).toBeInstanceOf(performance_monitor_1.PerformanceMonitor);
        });
    });
    describe('recordMetric', () => {
        test('should record a performance metric successfully', () => {
            const metric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 1500, 'ms', { operation: 'test' });
            expect(metric).toBeDefined();
            expect(metric.agentId).toBe('test-agent');
            expect(metric.metricType).toBe(performance_monitor_1.MetricType.EXECUTION_TIME);
        ...
        test('should assign appropriate status based on thresholds', () => {
            const normalMetric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 1000);
            const warningMetric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 7000);
            const criticalMetric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 20000);
        ...
        test('should handle different metric types', () => {
            const types = [
                performance_monitor_1.MetricType.EXECUTION_TIME,
                performance_monitor_1.MetricType.MEMORY_USAGE,
                performance_monitor_1.MetricType.CPU_USAGE,
                performance_monitor_1.MetricType.THROUGHPUT,
                performance_monitor_1.MetricType.ERROR_RATE
            ];
            types.forEach(type => {
        ...
        test('should emit metricRecorded event', (done) => {
            ...
            performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 1000);
        });
    });
    ...
    describe('recordThroughput', () => {
        test('should calculate throughput per minute correctly', () => {
            const metric = performanceMonitor.recordThroughput('test-agent', 100, // operations completed
            60000, // 1 minute
            { batchSize: 100 });
            expect(metric.value).toBe(100); // 100 ops/min
            expect(metric.unit).toBe('ops/min');
            expect(metric.metricType).toBe(performance_monitor_1.MetricType.THROUGHPUT);
        });
        test('should handle different time windows', () => {
            const metric = performanceMonitor.recordThroughput('test-agent', 50, // operations completed
            30000 // 30 seconds
            );
    ...
    describe('recordErrorRate', () => {
        test('should calculate error rate percentage correctly', () => {
            const metric = performanceMonitor.recordErrorRate('test-agent', 5, // errors
            100 // total operations
            );
            expect(metric.value).toBe(5);
            expect(metric.unit).toBe('%');
            expect(metric.metricType).toBe(performance_monitor_1.MetricType.ERROR_RATE);
        });
        test('should handle zero total operations', () => {
            const metric = performanceMonitor.recordErrorRate('test-agent', 0, 0);
    ...
    describe('getAgentSummary', () => {
        ...
        test('should calculate agent summary after recording metrics', () => {
            performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 1000);
            performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 1500);
            performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.MEMORY_USAGE, 256);
    ...
    describe('getMetrics', () => {
        beforeEach(() => {
            ...
            performanceMonitor.recordMetric('agent1', performance_monitor_1.MetricType.EXECUTION_TIME, 1000);
            performanceMonitor.recordMetric('agent2', performance_monitor_1.MetricType.MEMORY_USAGE, 256);
            performanceMonitor.recordMetric('agent1', performance_monitor_1.MetricType.THROUGHPUT, 50);
        });
        ...
        test('should filter by metric type', () => {
            const metrics = performanceMonitor.getMetrics(60000, undefined, performance_monitor_1.MetricType.EXECUTION_TIME);
            expect(metrics.length).toBe(1);
            expect(metrics[0].metricType).toBe(performance_monitor_1.MetricType.EXECUTION_TIME);
        });
        test('should filter by both agent ID and metric type', () => {
            const metrics = performanceMonitor.getMetrics(60000, 'agent1', performance_monitor_1.MetricType.THROUGHPUT);
            expect(metrics.length).toBe(1);
            expect(metrics[0].agentId).toBe('agent1');
            expect(metrics[0].metricType).toBe(performance_monitor_1.MetricType.THROUGHPUT);
        });
    });
    ...
    describe('generateReport', () => {
        beforeEach(() => {
            ...
            for (let i = 0; i < 10; i++) {
                performanceMonitor.recordMetric('agent1', performance_monitor_1.MetricType.EXECUTION_TIME, 1000 + i * 100);
                performanceMonitor.recordMetric('agent2', performance_monitor_1.MetricType.MEMORY_USAGE, 200 + i * 10);
            }
        });
    ...
    describe('Alert System', () => {
        test('should trigger alerts for critical metrics', (done) => {
            ...
            performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, 25000);
        });
    ...
    describe('Edge Cases', () => {
        test('should handle negative values appropriately', () => {
            const metric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, -100);
        ...
        test('should handle extremely large values', () => {
            ...
            const metric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, largeValue);
            expect(metric.value).toBe(largeValue);
            expect(metric.status).toBe('critical');
        });
        test('should handle special numeric values', () => {
            const infinityMetric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, Infinity);
            const nanMetric = performanceMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.MEMORY_USAGE, NaN);
    ...
    describe('Memory Management', () => {
        test('should limit metrics in memory', () => {
            ...
            const smallLimitMonitor = new performance_monitor_1.PerformanceMonitor();
            // Record more metrics than the limit
            for (let i = 0; i < 1500; i++) {
                smallLimitMonitor.recordMetric('test-agent', performance_monitor_1.MetricType.EXECUTION_TIME, i);
            }
...
describe('Global Performance Monitor', () => {
    test('should return singleton instance', () => {
        const monitor1 = (0, performance_monitor_1.getPerformanceMonitor)();
        const monitor2 = (0, performance_monitor_1.getPerformanceMonitor)();
...
describe('timeOperation utility', () => {
    test('should time async operations correctly', async () => {
        ...
        const result = await (0, performance_monitor_1.timeOperation)('test-agent', 'async-test', testOperation);
        expect(result).toBe('done');
        // Check that metrics were recorded
        const monitor = (0, performance_monitor_1.getPerformanceMonitor)();
        const metrics = monitor.getMetrics(60000, 'test-agent', performance_monitor_1.MetricType.EXECUTION_TIME);
    ...
    test('should handle operation failures', async () => {
        ...
        await expect((0, performance_monitor_1.timeOperation)('test-agent', 'failing-test', failingOperation)).rejects.toThrow('Test error');
