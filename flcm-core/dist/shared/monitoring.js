...
class MonitoringSystem extends events_1.EventEmitter {
     ...
     */
    async collectFlcmMetrics(timestamp) {
        try {
            // Agent status
            ...
            if (fs.existsSync(pipelineDir)) {
                const components = fs.readdirSync(pipelineDir).filter(f => f.endsWith('.ts') || f.endsWith('.js'));
    ...
    checkThresholds() {
        ...
        if (memoryMetrics && memoryMetrics.length > 0) {
            ...
            if (latest.value > this.config.thresholds.memory) {
                this.createAlert('memory-high', 'warning', `Memory usage is ${latest.value.toFixed(1)}% (threshold: ${this.config.thresholds.memory}%)`, 'system.memory.percentage', this.config.thresholds.memory, latest.value);
            }
        }
        // Disk threshold
        ...
        if (diskMetrics && diskMetrics.length > 0) {
            ...
            if (latest.value > this.config.thresholds.disk) {
                this.createAlert('disk-high', latest.value > 95 ? 'critical' : 'warning', `Disk usage is ${latest.value.toFixed(1)}% (threshold: ${this.config.thresholds.disk}%)`, 'system.disk.percentage', this.config.thresholds.disk, latest.value);
            }
        }
        ...
        if (cpuMetrics && cpuMetrics.length > 0) {
            ...
            if (cpuPercentage > this.config.thresholds.cpu) {
                this.createAlert('cpu-high', cpuPercentage > 150 ? 'critical' : 'warning', `CPU load is ${cpuPercentage.toFixed(1)}% (threshold: ${this.config.thresholds.cpu}%)`, 'system.cpu.load1', this.config.thresholds.cpu, cpuPercentage);
            }
        }
    }
    /**
     * Create an alert
     */
    createAlert(id, level, message, metric, threshold, value) {
