...
exports.timeOperation = exports.getPerformanceMonitor = exports.PerformanceMonitor = exports.MetricType = void 0;
...
(function (MetricType) {
    ...
    MetricType["QUEUE_SIZE"] = "queue_size";
    MetricType["RESPONSE_TIME"] = "response_time";
    MetricType["RESOURCE_UTILIZATION"] = "resource_utilization";
...
class PerformanceMonitor extends events_1.EventEmitter {
     ...
     */
    recordMetric(agentId, metricType, value, unit = 'ms', context = {}, taskId, tags = []) {
        const metric = {
     ...
     */
    startTiming(agentId, operation, context = {}) {
        ...
        return {
            stop: (taskId, tags = []) => {
                ...
                const timeMetric = this.recordMetric(agentId, MetricType.EXECUTION_TIME, executionTime, 'ms', { ...context, operation, memoryDelta }, taskId, [...tags, operation]);
                // Record memory usage if significant
                if (Math.abs(memoryDelta) > 1024 * 1024) { // 1MB threshold
                    this.recordMetric(agentId, MetricType.MEMORY_USAGE, endMemory.heapUsed / (1024 * 1024), 'MB', { ...context, operation, memoryDelta }, taskId, [...tags, operation]);
                }
                return timeMetric;
            }
        };
     ...
     */
    recordThroughput(agentId, operationsCompleted, timeWindowMs, context = {}) {
        const throughputPerMinute = (operationsCompleted / timeWindowMs) * 60000;
        return this.recordMetric(agentId, MetricType.THROUGHPUT, throughputPerMinute, 'ops/min', { ...context, operationsCompleted, timeWindowMs });
    }
    /**
     * Record error rate
     */
    recordErrorRate(agentId, errorCount, totalOperations, context = {}) {
        const errorRate = totalOperations > 0 ? (errorCount / totalOperations) * 100 : 0;
        return this.recordMetric(agentId, MetricType.ERROR_RATE, errorRate, '%', { ...context, errorCount, totalOperations });
    }
    /**
     ...
     */
    getMetrics(timeRangeMs, agentId, metricType) {
        const cutoff = new Date(Date.now() - timeRangeMs);
        return this.metrics.filter(metric => {
            if (metric.timestamp < cutoff)
                return false;
            if (agentId && metric.agentId !== agentId)
                return false;
     ...
     */
    generateReport(timeRangeMs = 24 * 60 * 60 * 1000) {
    ...
    evaluateStatus(metricType, value) {
    ...
    updateAgentSummary(agentId, metric) {
    ...
    calculateTrends(agentId) {
        const recentMetrics = this.getMetrics(60 * 60 * 1000, agentId); // Last hour
        const olderMetrics = this.getMetrics(2 * 60 * 60 * 1000, agentId).filter(m => m.timestamp < new Date(Date.now() - 60 * 60 * 1000)); // Hour before that
        return {
            executionTime: this.compareTrend(this.getAverageValue(recentMetrics, MetricType.EXECUTION_TIME), this.getAverageValue(olderMetrics, MetricType.EXECUTION_TIME)),
            memoryUsage: this.compareTrend(this.getAverageValue(recentMetrics, MetricType.MEMORY_USAGE), this.getAverageValue(olderMetrics, MetricType.MEMORY_USAGE)),
            errorRate: this.compareTrend(this.getAverageValue(recentMetrics, MetricType.ERROR_RATE), this.getAverageValue(olderMetrics, MetricType.ERROR_RATE))
        };
    }
    getAverageValue(metrics, type) {
    ...
    compareTrend(recent, older) {
        ...
        if (change > 0.1)
            return 'degrading'; // 10% worse
        if (change < -0.1)
            return 'improving'; // 10% better
        return 'stable';
    }
    calculateOverallHealthScore(agents) {
    ...
    generateRecommendations(metrics, agents, systemHealth) {
    ...
    collectSystemMetrics() {
        ...
        this.recordMetric('system', MetricType.MEMORY_USAGE, systemHealth.memory.percentage, '%', { total: systemHealth.memory.total, used: systemHealth.memory.used });
        this.recordMetric('system', MetricType.CPU_USAGE, systemHealth.cpu.loadAverage[0], 'load', { loadAverage: systemHealth.cpu.loadAverage });
    }
    ...
    loadExistingMetrics() {
        try {
            if (fs.existsSync(this.metricsFilePath)) {
                ...
                this.metrics = parsed.map((metric) => ({
                    ...metric,
 ...
 */
function getPerformanceMonitor(thresholds) {
 ...
 */
function timeOperation(agentId, operation, fn, context) {
