...
export interface VoiceProfile {
  ...
  tone: {
    ...
    emotion: number;          // 0-1 (neutral to emotional)
    authority: number;        // 0-1 (tentative to authoritative)
...
export class VoiceDNAAnalyzer {
  private readonly DEFAULT_PROFILE: VoiceProfile = {
    linguistic: {
   ...
   */
  private analyzeLinguistic(samples: string[]): VoiceProfile['linguistic'] {
   ...
   */
  private analyzeTone(samples: string[]): VoiceProfile['tone'] {
   ...
   */
  private analyzeStyle(samples: string[]): VoiceProfile['style'] {
    const allText = samples.join(' ');
    ...
    const metaphorUsage: 'rare' | 'occasional' | 'frequent' = 
      metaphorCount < 2 ? 'rare' : metaphorCount < 5 ? 'occasional' : 'frequent';
    ...
    const conversational = conversationalIndicators.filter(c => 
   ...
   */
  private analyzeStructure(samples: string[]): VoiceProfile['structure'] {
    ...
    const paragraphLengths = paragraphs.map(p => 
      this.extractSentences(p).length
    );
    ...
    // List usage
    const listCount = samples.filter(s => 
      /^[\*\-\+•]\s+/gm.test(s) || /^\d+\.\s+/gm.test(s)
    ).length;
    const listUsage: 'minimal' | 'moderate' | 'heavy' = 
      listCount < 2 ? 'minimal' : listCount < 5 ? 'moderate' : 'heavy';
   ...
   */
  private extractSignatures(samples: string[]): VoiceProfile['signatures'] {
    const allText = samples.join(' ');
    const sentences = this.extractSentences(allText);
    ...
    const usedTransitions = transitionWords.filter(t => 
  ...
  private assessVocabularyLevel(uniqueWords: Set<string>): 'simple' | 'moderate' | 'advanced' | 'expert' {
    const advancedWords = [
    ...
    if (advancedCount >= 5) return 'expert';
    if (advancedCount >= 3) return 'advanced';
    if (advancedCount >= 1) return 'moderate';
    return 'simple';
  }

  private detectOpeningStyle(sentences: string[]): VoiceProfile['structure']['openingStyle'] {
    const patterns = {
    ...
    return (style as VoiceProfile['structure']['openingStyle']) || 'statement';
  }

  private detectClosingStyle(sentences: string[]): VoiceProfile['structure']['closingStyle'] {
    const patterns = {
    ...
    return (style as VoiceProfile['structure']['closingStyle']) || 'summary';
  }
  ...
  private extractPattern(sentence: string): string {
  ...
  private generateRecommendations(profile: VoiceProfile): string[] {
  ...
  private combineSentences(content: string): string {
  ...
  private addTransitions(content: string, transitions: string[]): string {
    // Add transitions between paragraphs
    const paragraphs = content.split('\n\n');
    const modified: string[] = [paragraphs[0]];
