...
export interface HashtagSet {
  primary: string[];      // Core hashtags related to main topic
  secondary: string[];    // Supporting hashtags
  trending: string[];     // Currently trending related hashtags
  branded: string[];      // Brand or campaign specific
  platform: string;       // Platform this set is optimized for
  totalCount: number;
...
export class HashtagGenerator {
  private trendingHashtags: Map<string, string[]>;
  private platformStyles: Map<string, HashtagConfig>;
  ...
  generate(
    ...
    const combined = this.combineAndLimit(
      primary,
      secondary,
      trending,
      branded,
  ...
  private getTrendingHashtags(content: string, keywords: string[]): string[] {
  ...
  private processBrandHashtags(brandTags: string[], config: HashtagConfig): string[] {
    return brandTags.map(tag => this.formatHashtag(tag, config.style));
  }

  private formatHashtag(text: string, style: string): string {
    // Clean the text
  ...
  private extractImportantPhrases(content: string): string[] {
  ...
  private identifyCategories(content: string, keywords: string[]): string[] {
  ...
  private getRelatedHashtags(theme: string): string[] {
  ...
  private combineAndLimit(
    ...
    branded: string[],
    limit: number
  ): { primary: string[]; secondary: string[]; trending: string[]; branded: string[] } {
