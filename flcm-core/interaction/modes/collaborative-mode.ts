...
import { CollaborativePattern } from '../types';
import { FrameworkSelector } from '../../v2/mentor/frameworks/framework-selector';
...
export class CollaborativeMode {
  ...
  private dialogueState: 'idle' | 'exploring' | 'creating' | 'publishing' = 'idle';
  
  constructor(context: SharedContext) {
    this.context = context;
    this.logger = new Logger('CollaborativeMode');
   ...
   */
  private extractFrameworkRequest(input: string): string | null {
   ...
   */
  private async handleFrameworkRequest(framework: string, input: string): Promise<string> {
...
Which would you like to use?`;
   ...
   */
  private matchPattern(input: string): CollaborativePattern | null {
   ...
   */
  private async handlePatternMatch(pattern: CollaborativePattern, input: string): Promise<string> {
    // Extract context
    const context = {
   ...
   */
  private async handleDialogueFlow(input: string): Promise<string> {
   ...
   * Handle idle state
   */
  private handleIdleState(input: string): string {
...
What would you like to work on? Just describe your goal naturally, or say "use [framework name]" to start with a specific framework.`;
   ...
   */
  private async handleExploringState(input: string): Promise<string> {
...
What's your next move?`;
   ...
   */
  private handleCreatingState(input: string): string {
    // Simplified for now
...
[Content creation flow would continue here]`;
   ...
   */
  private handlePublishingState(input: string): string {
    // Simplified for now
...
Just name the platform(s) and I'll adapt your content accordingly.`;
   ...
   */
  private async handleAction(action: string, input: string, context: any): Promise<string> {
   ...
   */
  private extractGoal(input: string): string | undefined {
    if (input.includes('understand')) return 'Deep understanding';
    if (input.includes('prioritize')) return 'Make decisions';
    if (input.includes('create')) return 'Create content';
    if (input.includes('publish')) return 'Publish content';
    if (input.includes('analyze')) return 'Analyze thoroughly';
    return undefined;
  }
   ...
   */
  private extractAudience(input: string): string | undefined {
