...
interface SWOTUSEDAnalysis {
  strengths: string[];
  weaknesses: string[];
  opportunities: string[];
  threats: string[];
  use: string[];          // How to use strengths
  stop: string[];         // What to stop doing
  exploit: string[];      // How to exploit opportunities
  defend: string[];       // How to defend against threats
}

export class SWOTUSEDFramework extends BaseFramework {
  constructor() {
    super();
...
Let's analyze ${context?.topic || 'your situation'} strategically and create an action plan.`;
  ...
  async process(answers: Record<string, any>, context?: FrameworkContext): Promise<FrameworkOutput> {
    // Parse SWOT elements
    const analysis: SWOTUSEDAnalysis = {
  ...
  private generateUseStrategies(strengths: string[], opportunities: string[], answers: any): string[] {
  ...
  private generateStopStrategies(weaknesses: string[], threats: string[], answers: any): string[] {
    const strategies: string[] = [];
  ...
  private generateExploitStrategies(opportunities: string[], strengths: string[], answers: any): string[] {
    const strategies: string[] = [];
  ...
  private generateDefendStrategies(threats: string[], strengths: string[], answers: any): string[] {
    const strategies: string[] = [];
  ...
  private generateInsights(analysis: SWOTUSEDAnalysis, answers: any): string[] {
    ...
    analysis.strengths.forEach(s => {
      analysis.opportunities.forEach(o => {
        if (this.areRelated(s, o)) soMatches++;
      });
    });
    ...
    analysis.weaknesses.forEach(w => {
      analysis.threats.forEach(t => {
        if (this.areRelated(w, t)) wtMatches++;
      });
    });
  ...
  private generateRecommendations(analysis: SWOTUSEDAnalysis, answers: any): string[] {
  ...
  private generateNextSteps(analysis: SWOTUSEDAnalysis, answers: any): string[] {
    const steps: string[] = [];
  ...
  private calculateStrategicPosition(analysis: SWOTUSEDAnalysis): string {
  ...
  private calculateConfidence(analysis: SWOTUSEDAnalysis): number {
    ...
    if (totalItems >= 12) confidence += 0.3;
    else if (totalItems >= 8) confidence += 0.2;
    else if (totalItems >= 4) confidence += 0.1;
    ...
    if (hasAllQuadrants) confidence += 0.2;
  ...
  private extractMissing(weakness: string): string {
    const match = weakness.match(/lack(?:\s+of)?\s+(.+)/i) || 
                  weakness.match(/no\s+(.+)/i);
    return match ? match[1] : 'critical capability';
  }
  
  private extractProblem(weakness: string): string {
    const match = weakness.match(/(?:poor|weak)\s+(.+)/i);
    return match ? match[1] : 'performance';
  }
  
  private extractRiskProfile(riskTolerance: string): number {
    if (riskTolerance.includes('Very Low')) return 0.1;
    if (riskTolerance.includes('Low')) return 0.3;
    if (riskTolerance.includes('Medium')) return 0.5;
    if (riskTolerance.includes('High') && !riskTolerance.includes('Very')) return 0.7;
    if (riskTolerance.includes('Very High')) return 0.9;
    return 0.5;
  }
