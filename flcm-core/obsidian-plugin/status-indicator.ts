...
export class SyncStatusIndicator {
   ...
   * Update status
   */
  updateStatus(status: SyncStatus, details?: string, count?: number): void {
  ...
  private updateDisplay(details?: string): void {
    ...
    this.statusBarItem.removeClass('flcm-status-idle', 'flcm-status-syncing', 
                                    'flcm-status-completed', 'flcm-status-error', 
                                    'flcm-status-conflict');
  ...
  showProgress(current: number, total: number): void {
    ...
    const textEl = this.statusBarItem.querySelector('.flcm-status-text') as HTMLElement;
    if (textEl) {
   ...
   * Set sync statistics
   */
  setSyncStats(stats: { successful: number; failed: number; conflicts: number }): void {
