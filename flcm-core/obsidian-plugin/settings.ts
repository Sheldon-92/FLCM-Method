...
export class FLCMSettingTab extends PluginSettingTab {
  ...
  private addStatusInformation(containerEl: HTMLElement): void {
  ...
  private async testConnection(): Promise<void> {
    if (!this.plugin.settings.flcmPath) {
      new Notice('Please set FLCM path first', 3000);
      return;
    }
    ...
    try {
      ...
      if (isConnected) {
        new Notice('✅ FLCM connection successful', 3000);
      } else {
        new Notice('❌ FLCM connection failed. Please check the path.', 5000);
      }
    } catch (error) {
  ...
  private async createFLCMDocument(): Promise<void> {
    ...
    try {
      ...
      new Notice(`Created FLCM document: ${fileName}`, 3000);
    } catch (error) {
      console.error('Failed to create FLCM document:', error);
      new Notice(`Failed to create document: ${error.message}`, 5000);
    }
  }
}