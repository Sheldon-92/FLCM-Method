...
export class SocraticQuestioningFramework extends ProgressiveFramework {
...
Each level deepens understanding and challenges thinking. Let's begin.`;
  ...
  private analyzeLevels(answers: Record<string, any>): SocraticLevel[] {
  ...
  private getAnsweredQuestions(answers: Record<string, any>, ...keywords: string[]): string[] {
    return Object.keys(answers).filter(key => 
      keywords.some(keyword => key.includes(keyword))
    );
  }
  
  private generateLevelInsights(answers: Record<string, any>, ...keywords: string[]): string[] {
    const insights: string[] = [];
    const relevantAnswers = Object.entries(answers).filter(([key]) => 
      keywords.some(keyword => key.includes(keyword))
    );
  ...
  private generateInsights(levels: SocraticLevel[], answers: Record<string, any>): string[] {
    const insights: string[] = [];
  ...
  private generateRecommendations(levels: SocraticLevel[], answers: Record<string, any>): string[] {
  ...
  private generateNextSteps(levels: SocraticLevel[], answers: Record<string, any>): string[] {
    const steps: string[] = [];
  ...
  private calculateUnderstandingDepth(levels: SocraticLevel[], answers: Record<string, any>): number {
    let depth = 0;
    ...
    if (avgAnswerLength > 200) depth += 0.2;
    else if (avgAnswerLength > 100) depth += 0.15;
    else if (avgAnswerLength > 50) depth += 0.1;
    
    // Critical thinking indicators (0-0.2)
    if (answers.challenge_assumption) depth += 0.05;
    if (answers.counter_evidence) depth += 0.05;
    if (answers.opposing_view) depth += 0.05;
    if (answers.better_questions) depth += 0.05;
    
    return Math.min(depth, 1);
  }
  
  private extractKeyRealizations(answers: Record<string, any>): string[] {
  ...
  private extractAssumptions(answers: Record<string, any>): string[] {
    if (!answers.identify_assumptions) return [];
  ...
  private extractPerspectives(answers: Record<string, any>): string[] {
  ...
  private extractCriticalQuestions(answers: Record<string, any>): string[] {
  ...
  private calculateAverageAnswerLength(answers: Record<string, any>): number {
  ...
  private getSocraticLevel(depth: number): string {
    if (depth < 0.2) return 'Surface - Basic questioning';
    if (depth < 0.4) return 'Shallow - Some critical thinking';
    if (depth < 0.6) return 'Developing - Good inquiry skills';
    if (depth < 0.8) return 'Deep - Strong analytical thinking';
