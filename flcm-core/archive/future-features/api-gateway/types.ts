...
export interface SSLConfiguration {
  ...
  ciphers: string[];
  dhParams?: string;
  redirect: boolean; // Redirect HTTP to HTTPS
}
...
export interface HealthThreshold {
  healthy: number; // consecutive successes
  unhealthy: number; // consecutive failures
}
...
export interface UpstreamTimeout {
  connect: number; // seconds
  send: number; // seconds
  receive: number; // seconds
}
...
export interface MutualTLSStatus {
  enabled: boolean;
  coverage: number; // percentage of services
  issues: MTLSIssue[];
}
...
export interface DataPlaneHealth {
  ...
  connectivity: number; // percentage of successful connections
}
...
export interface MeshSecurityMetrics {
  mtlsAdoption: number; // percentage
