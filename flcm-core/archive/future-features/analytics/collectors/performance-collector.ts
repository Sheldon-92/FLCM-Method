...
import { MetricCollector, PerformanceMetrics } from '../types';
...
export class PerformanceCollector implements MetricCollector {
  ...
  private maxSamples: number = 1000; // Keep last 1000 samples per operation
  
  constructor() {
    this.metrics = new Map();
    this.timings = new Map();
    this.logger = new Logger('PerformanceCollector');
  }
   ...
   */
  private updateMetrics(version: '1.0' | '2.0', operation: string): void {
    const key = `${version}:${operation}`;
  ...
  async collect(): Promise<Record<string, any>> {
    ...
    const operationMetrics: Map<string, any> = new Map();
   ...
   */
  private getOperationsList(version: string): string[] {
   ...
   */
  private getOverallAverage(version: string): number {
   ...
   */
  private getTotalSamples(version: string): number {
    let total = 0;
