...
import { Cohort, CohortRule, UserContext } from './types';
import { Logger } from '../shared/utils/logger';
import * as crypto from 'crypto';

export class CohortManager {
  private cohorts: Map<string, Cohort>;
  private userCohortCache: Map<string, string[]>;
  private logger: Logger;
  
  constructor() {
    this.logger = new Logger('CohortManager');
   ...
   */
  async getUserCohorts(userId: string, context?: UserContext): Promise<string[]> {
    // Check cache first
   ...
   */
  private evaluateRules(rules: CohortRule[], context: UserContext): boolean {
   ...
   */
  private evaluateRule(rule: CohortRule, value: any): boolean {
   ...
   */
  async isUserInCohort(userId: string, cohortName: string, context?: UserContext): Promise<boolean> {
  ...
  importCohorts(config: any): void {
    try {
      ...
      Object.entries(cohorts).forEach(([name, data]: [string, any]) => {
        const cohort: Cohort = {
          name,
