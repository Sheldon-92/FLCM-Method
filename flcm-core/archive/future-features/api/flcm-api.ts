...
import { PipelineOrchestrator, WorkflowConfig, WorkflowResult } from '../workflow/pipeline-orchestrator';
 ...
 */
app.post('/api/workflows/start', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      topic,
      mode = 'standard',
    ...
    workflow.on('quickmode:error', (data: any) => {
      const wf = workflows.get(workflowId);
      if (wf) {
        wf.status = 'failed';
    ...
    workflow.on('standardmode:error', (data: any) => {
      const wf = workflows.get(workflowId);
      if (wf) {
        wf.status = 'failed';
        wf.error = data.error.message;
 ...
 * Get workflow result
 */
app.get('/api/workflows/:id/result', (req: Request, res: Response) => {
  const workflowId = req.params.id;
  const workflow = workflows.get(workflowId);
 ...
 * Cancel workflow
 */
app.post('/api/workflows/:id/cancel', (req: Request, res: Response) => {
  const workflowId = req.params.id;
  const workflow = workflows.get(workflowId);
 ...
 */
app.post('/api/agents/:name/execute', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const agentName = req.params.name;
 ...
 */
app.post('/api/content/preview', (req: Request, res: Response) => {
 ...
 * Export content
 */
app.post('/api/content/export', (req: Request, res: Response) => {
