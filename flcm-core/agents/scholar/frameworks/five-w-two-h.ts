...
const logger = createLogger('FiveW2HFramework');

export interface FiveW2HResult {
  what: string[];     // What is it about?
  when: string[];     // When does it happen?
  where: string[];    // Where does it occur?
  who: string[];      // Who is involved?
  why: string[];      // Why does it happen?
  how: string[];      // How does it work?
  howMuch: string[];  // How much/many?
}

export class FiveW2HFramework {
  /**
   * Analyze content using 5W2H framework
   */
  async analyze(content: string): Promise<FiveW2HResult> {
    try {
  ...
  private extractWhat(content: string): string[] {
  ...
  private extractWhen(content: string): string[] {
    const results: string[] = [];
  ...
  private extractWhere(content: string): string[] {
    const results: string[] = [];
    
    // Location patterns
    const locationPatterns = [
      /in\s+[A-Z][a-z]+/g, // In Location
      /at\s+[A-Z][a-z]+/g, // At Location
  ...
  private extractWho(content: string): string[] {
    const results: string[] = [];
  ...
  private extractWhy(content: string): string[] {
    const results: string[] = [];
  ...
  private extractHow(content: string): string[] {
    const results: string[] = [];
    
    // Method patterns
    const howPatterns = [
      /by\s+\w+ing/gi, // by doing
  ...
  private extractHowMuch(content: string): string[] {
    const results: string[] = [];
  ...
  extractInsights(results: FiveW2HResult): string[] {
    ...
    for (const { key, label } of categories) {
      const items = results[key as keyof FiveW2HResult];
