...
const mockProcessorTiming = (baseTime: number) => ({
...
const mockFrameworkTiming = (baseTime: number, insights: string[]) => ({
...
describe('Scholar Agent Performance Benchmarks', () => {
  ...
  describe('Core Performance Requirements', () => {
    ...
    it('should process different input types within acceptable time limits', async () => {
      const testCases = [
        { type: SourceType.TEXT, maxTime: 1000 },
        { type: SourceType.PDF, maxTime: 3000 },
        { type: SourceType.WEBPAGE, maxTime: 4000 },
        { type: SourceType.CODE, maxTime: 2000 },
      ];
  ...
  describe('Framework Performance Optimization', () => {
    it('should benefit from parallel framework execution', async () => {
      ...
      const frameworks: Framework[] = ['SWOT-USED', 'SCAMPER', 'Socratic', '5W2H'];

      // Test parallel execution
      const parallelInput: ScholarInput = {
        source: content,
        frameworks,
        options: { parallelFrameworks: true },
      };
      ...
      const sequentialInput: ScholarInput = {
        source: content,
        frameworks,
    ...
    it('should show performance improvement with framework selection', async () => {
      ...
      const allFrameworksInput: ScholarInput = {
        source: content,
        frameworks: ['SWOT-USED', 'SCAMPER', 'Socratic', '5W2H', 'Pyramid'],
      };
      ...
      const selectiveInput: ScholarInput = {
        source: content,
        frameworks: ['SWOT-USED', 'Socratic'],
      };
  ...
  describe('Resource Usage Optimization', () => {
    ...
    it('should maintain consistent performance across input types', async () => {
      const inputTypes: SourceType[] = [
        SourceType.TEXT,
        SourceType.MARKDOWN,
        SourceType.CODE,
      ];

      const durations: Record<SourceType, number> = {} as any;
  ...
  describe('Error Handling Performance', () => {
    it('should fail fast for invalid inputs', async () => {
      const invalidInputs: ScholarInput[] = [
        { source: '' }, // Empty source
      ...
      const baselineTests = [
        {
          ...
          input: { source: 'Simple text', frameworks: ['SWOT-USED'] as Framework[] },
          maxTime: 2000,
        },
        {
          name: 'Multi-framework analysis',
          input: { source: 'Complex content', frameworks: ['SWOT-USED', 'SCAMPER', 'Socratic'] as Framework[] },
          maxTime: 4000,
        },
        {
          name: 'Comprehensive analysis',
          input: { source: 'Very detailed content', frameworks: ['SWOT-USED', 'SCAMPER', 'Socratic', '5W2H', 'Pyramid'] as Framework[] },
          maxTime: 5000,
        },
      ];
