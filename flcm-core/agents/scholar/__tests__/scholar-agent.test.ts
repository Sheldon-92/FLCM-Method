...
import { ScholarAgent, ScholarInput, PerformanceMetrics } from '../index';
...
describe('ScholarAgent', () => {
  ...
  describe('Agent Initialization', () => {
    ...
    it('should have correct capabilities', () => {
      ...
      expect(multiSource?.inputTypes).toEqual(Object.values(SourceType));
  ...
  describe('Input Type Detection', () => {
    it('should detect webpage URLs correctly', () => {
      expect(agent.detectInputType('https://example.com')).toBe(SourceType.WEBPAGE);
      expect(agent.detectInputType('http://example.com')).toBe(SourceType.WEBPAGE);
    });

    it('should detect PDF files correctly', () => {
      expect(agent.detectInputType('document.pdf')).toBe(SourceType.PDF);
    });

    it('should detect markdown files correctly', () => {
      expect(agent.detectInputType('readme.md')).toBe(SourceType.MARKDOWN);
    });

    it('should detect video files correctly', () => {
      expect(agent.detectInputType('video.mp4')).toBe(SourceType.VIDEO);
      expect(agent.detectInputType('video.avi')).toBe(SourceType.VIDEO);
    });

    it('should detect audio files correctly', () => {
      expect(agent.detectInputType('audio.mp3')).toBe(SourceType.AUDIO);
      expect(agent.detectInputType('audio.wav')).toBe(SourceType.AUDIO);
    });

    it('should detect image files correctly', () => {
      expect(agent.detectInputType('image.jpg')).toBe(SourceType.IMAGE);
      expect(agent.detectInputType('image.png')).toBe(SourceType.IMAGE);
    });

    it('should detect code files correctly', () => {
      expect(agent.detectInputType('script.js')).toBe(SourceType.CODE);
      expect(agent.detectInputType('app.py')).toBe(SourceType.CODE);
      expect(agent.detectInputType('component.ts')).toBe(SourceType.CODE);
    });

    it('should detect spreadsheet files correctly', () => {
      expect(agent.detectInputType('data.xlsx')).toBe(SourceType.SPREADSHEET);
      expect(agent.detectInputType('data.csv')).toBe(SourceType.SPREADSHEET);
    });

    it('should default to text for unknown types', () => {
      expect(agent.detectInputType('unknown.xyz')).toBe(SourceType.TEXT);
      expect(agent.detectInputType('plain text')).toBe(SourceType.TEXT);
    });

    it('should handle buffer inputs', () => {
      const buffer = Buffer.from('test data');
      expect(agent.detectInputType(buffer)).toBe(SourceType.TEXT);
    });
  });
  ...
  describe('Framework Analysis', () => {
    ...
    it('should throw error for unknown framework', async () => {
      await expect(
        agent.applyFramework(testContent, 'UnknownFramework' as Framework)
      ).rejects.toThrow('Framework not implemented: UnknownFramework');
    });

    it('should apply multiple frameworks in parallel', async () => {
      const startTime = Date.now();
      const results = await agent['applyFrameworks'](
        testContent,
        ['SWOT-USED', 'SCAMPER', 'Socratic'],
        true
      );
  ...
  describe('Main Analysis Method', () => {
    it('should perform complete text analysis successfully', async () => {
      ...
      expect(result.metadata.sourceType).toBe(SourceType.TEXT);
    ...
    it('should handle web URL analysis', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.metadata.sourceType).toBe(SourceType.WEBPAGE);
    });
    ...
    it('should validate input requirements', async () => {
      const invalidInput: ScholarInput = {
        source: '', // Empty source
      };
  ...
  describe('Performance Metrics', () => {
    it('should track analysis metrics correctly', async () => {
      ...
      expect(metrics.inputTypeDistribution[SourceType.TEXT]).toBe(1);
    });

    it('should track error rates correctly', async () => {
      // Force an error by providing invalid framework
      try {
        await agent.applyFramework('test', 'InvalidFramework' as Framework);
      } catch (error) {
        // Expected error
      }
