...
export class EnhancedErrorHandler extends EventEmitter {
  ...
  private errorStrategies = new Map<ErrorType, ErrorHandlingStrategy>();
  private errorHistory: EnhancedErrorContext[] = [];
  private userSessions = new Map<string, EnhancedErrorContext[]>();
   ...
   */
  getErrorInsights(): {
    topErrors: Array<{ type: ErrorType; count: number; severity: ErrorSeverity }>;
    errorTrends: Array<{ date: string; count: number }>;
    recoverySuccess: { [key in ErrorType]: number };
    userImpactAnalysis: {
    ...
    // Top errors
    const errorCounts = new Map<ErrorType, { count: number; severity: ErrorSeverity }>();
    ...
    const recoverySuccess = {} as { [key in ErrorType]: number };
    Object.values(ErrorType).forEach(type => {
      const typeErrors = recentErrors.filter(e => e.errorType === type);
      const successfulRecoveries = typeErrors.filter(e => 
        this.recoveryManager.getErrorHistory().some(h => 
          h.agent === e.agent && h.operation === e.operation && h.timestamp > e.timestamp
        )
      );
    ...
    const highImpactErrors = recentErrors.filter(e => 
      e.userImpact === 'high' || e.userImpact === 'critical'
    ).length;
    ...
    const avgResolutionTime = recentErrors
      ...
      .reduce((sum, e) => sum + (e.metadata.resolutionTime as number), 0) / 
...
`;
  ...
  private async enrichErrorContext(
    error: Error,
    partialContext: Partial<EnhancedErrorContext>
  ...
  private assessUserImpact(errorType: ErrorType, error: Error): EnhancedErrorContext['userImpact'] {
    switch (errorType) {
  ...
  private determineSeverity(errorType: ErrorType, userImpact: EnhancedErrorContext['userImpact']): ErrorSeverity {
    if (userImpact === 'critical') return ErrorSeverity.CRITICAL;
    if (userImpact === 'high') return ErrorSeverity.HIGH;
    if (userImpact === 'medium') return ErrorSeverity.MEDIUM;
    return ErrorSeverity.LOW;
  }

  private generateUserMessage(error: Error, errorType: ErrorType): string {
  ...
  private generateTechnicalDetails(error: Error): string {
  ...
  private getErrorStrategy(errorType: ErrorType): ErrorHandlingStrategy {
  ...
  private async analyzeError(context: EnhancedErrorContext): Promise<ErrorAnalysisResult> {
    ...
    return {
      ...
      impactAssessment: {
        ...
        systemStability: context.severity === ErrorSeverity.CRITICAL ? 'critical' : 'stable'
      },
      recommendedActions: [
  ...
  private getUserMessage(context: EnhancedErrorContext, strategy: ErrorHandlingStrategy): string {
  ...
  private shouldAutoRetry(context: EnhancedErrorContext, strategy: ErrorHandlingStrategy): boolean {
  ...
  private calculateBackoffDelay(attempt: number, strategy: ErrorHandlingStrategy['backoffStrategy']): number {
  ...
  private setupEventListeners(): void {
    this.on('error-occurred', (context: EnhancedErrorContext) => {
  ...
  private getRecentErrors(timeWindowMs: number): EnhancedErrorContext[] {
    const cutoff = new Date(Date.now() - timeWindowMs);
    return this.errorHistory.filter(error => error.timestamp >= cutoff);
  }

  private updateUserSession(context: EnhancedErrorContext): void {
  ...
  private generateUserRecommendations(errors: EnhancedErrorContext[]): string[] {
...
export const enhancedErrorHandler = new EnhancedErrorHandler(
  require('./recovery-manager').recoveryManager
);