...
import { CreatorAgent, CreationOptions, VoiceDNAProfile, ConsistencyScore } from '../index';
...
describe('CreatorAgent', () => {
  ...
  beforeEach(() => {
    ...
    mockInsightsDocument = createInsightsDocument(
      ...
      ['SWOT-USED', 'Socratic'],
      'Test insights content with key findings and recommendations.',
      'Scholar Agent'
    );
  ...
  describe('Voice DNA System', () => {
    ...
    it('should require minimum samples for Voice DNA extraction', async () => {
      ...
      await expect(agent.extractVoiceDNA(insufficientSamples)).rejects.toThrow(
        'Minimum 5 samples required for Voice DNA extraction'
      );
    });
    ...
    it('should detect low consistency scores', async () => {
      ...
      const formalProfile: VoiceDNAProfile = {
        id: 'formal-profile',
        userId: 'user-123',
        created: new Date(),
        updated: new Date(),
  ...
  describe('Content Quality and Structure', () => {
    ...
    it('should integrate insights meaningfully', async () => {
      ...
      mockInsightsDocument.keyFindings.forEach(finding => {
        ...
        const contentLower = result.content.toLowerCase();
        const matchedWords = findingWords.filter(word => 
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle empty insights gracefully', async () => {
      const emptyInsights = createInsightsDocument(
        { type: 'text' as any, path: 'empty', hash: 'empty' },
        ['SWOT-USED'],
        '',
        'Scholar Agent'
      );
  ...
  describe('Agent Request Processing', () => {
    ...
    it('should handle request processing errors', async () => {
      const request = {
        ...
        timestamp: new Date(),
        data: {
          insights: null, // Invalid insights
          options: {
            mode: 'standard' as CreationMode,
          },
        },
  ...
  describe('Statistics and Metrics', () => {
    it('should track creation statistics correctly', async () => {
      const options1: CreationOptions = { mode: 'quick' };
      const options2: CreationOptions = { mode: 'standard' };
      const options3: CreationOptions = { mode: 'custom' };
      ...
      const stats = (agent as any).creationStats;
    ...
    it('should update average metrics correctly', async () => {
      const options: CreationOptions = { mode: 'quick' };
