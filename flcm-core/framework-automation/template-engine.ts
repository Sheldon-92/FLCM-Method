...
export class TemplateEngine {
   ...
   */
  private templateToMarkdown(template: FrameworkTemplate, personalizations: Record<string, any>): string {
   ...
   */
  private applyCustomizations(template: FrameworkTemplate, customizations: Partial<TemplateCustomization>): FrameworkTemplate {
    if (!customizations.modifications) return template;
   ...
   * Generate prompts from sections
   */
  private generatePromptsFromSections(sections: TemplateSection[]): TemplatePrompt[] {
  ...
  getUserCustomizations(userId: string): TemplateCustomization[] {
    const userCustomizations: TemplateCustomization[] = [];
  ...
  private getTemplateDistribution(): Record<string, number> {
  ...
  private calculateAvgEffectiveness(): number {
